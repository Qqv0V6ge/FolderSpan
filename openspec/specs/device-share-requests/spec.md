# device-share-requests Specification

## Purpose
TBD - created by archiving change replace-share-sse-with-polling. Update Purpose after archive.
## Requirements
...
#### Scenario: Request rejected when receive is disabled for unknown devices
- **WHEN** a device share request is received from a device without a saved receive-share record
- **AND** the "allow other devices to share to me" setting is disabled
- **THEN** the endpoint returns a status of REJECTED
- **AND** no pending approval request is created
