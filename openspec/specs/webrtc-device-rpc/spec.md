# webrtc-device-rpc Specification

## Purpose
TBD - created by archiving change add-webrtc-device-rpc. Update Purpose after archive.
## Requirements
...
WebRTC device RPC envelopes SHALL be encoded as plain ProtoBuf bytes on the WebRTC data channel and SHALL NOT be wrapped in application-level symmetric encryption.

#### Scenario: RPC envelope exchange
- **WHEN** a peer sends a WebRTC device RPC envelope over an established data channel
