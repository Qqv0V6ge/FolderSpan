...
The system SHALL expose a TLS-protected counterpart for link-share browser routes and static assets when streamed browser saving requires HTTPS, while keeping the user-facing default link-share entry on HTTP.
...
#### Scenario: HTTPS serves same link-share routes
...
- **THEN** the HTTPS endpoint serves the same link-share page, static assets, JSON listings, and authorized file downloads
- **AND** the endpoint uses the app's self-signed TLS identity for that device.

### Requirement: Link-share HTTPS consent guidance
The system SHALL require an explicit app-level consent step before app-initiated navigation from HTTP to HTTPS for streamed batch saving, and SHALL remember consent for the same endpoint identity.

#### Scenario: User starts streamed download from HTTP
- **GIVEN** the user is browsing the link-share page over HTTP
- **AND** no remembered consent exists for the current host, ports, and TLS fingerprint
...
- **GIVEN** the user has already consented for the current host, ports, and TLS fingerprint
...
#### Scenario: Direct HTTPS open without app consent
- **GIVEN** a browser request reaches the HTTPS link-share endpoint without remembered app consent
- **WHEN** the requested resource is a browser page rather than a static asset or API/file request
- **THEN** the server redirects to the matching HTTP URL with a marker indicating HTTPS was opened directly
- **AND** the HTTP page explains the expected self-signed certificate warning
- **AND** asks whether the user wants to continue back to HTTPS for streamed saving.
