...
The MCP listener SHALL advertise loopback and non-loopback interface URLs, SHALL prefer HTTPS URLs in the UI, and SHALL visibly warn that plaintext HTTP exposes Bearer Tokens and file parameters to LAN observers. Requests with an `Origin` header SHALL be accepted only when the Origin is loopback or is explicitly allowed in MCP settings. Host and Origin validation SHALL prevent DNS-rebinding access to an unadvertised host.
...
- **WHEN** a request includes an Origin that is neither loopback nor in the configured allowlist
- **THEN** the server rejects the request before Token or tool processing

### Requirement: MCP management entry and lifecycle
`ToolboxScreen` SHALL contain an MCP entry that opens a Material 3 management screen. The screen SHALL expose enabled/running/error status, public port, advertised addresses, start/stop/restart actions, allowed Origins, and Token create/edit/disable/delete operations. Port or listener-affecting changes SHALL require a controlled restart and SHALL preserve unrelated file-share services.
