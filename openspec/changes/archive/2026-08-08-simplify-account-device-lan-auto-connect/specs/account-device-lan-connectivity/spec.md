...
The system SHALL evaluate each device returned by the existing LAN discovery flow against the current trusted account-device snapshot.
It SHALL automatically start the existing direct device connection flow only when the discovered device is a remote device registered to the current account and its discovery identity proof is fresh and valid.
...
- **WHEN** LAN discovery receives a device whose registered identity and fresh discovery proof match the trusted account-device snapshot
- **AND** the device is not connected or being connected
- **THEN** the system starts one automatic direct connection attempt
...
- **WHEN** repeated scans discover the same eligible device while it is connected or being connected
