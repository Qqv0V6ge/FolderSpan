...
private var fileShareServiceStartingOrRunning = false
private val fileShareServiceLock = NSLock()
private val fileShareRawTlsServer = RawTlsHttpServer()

actual suspend fun startFileShareService() {
    val shouldStart = fileShareServiceLock.withLock {
        if (fileShareServiceStartingOrRunning) {
            false
        } else {
            fileShareServiceStartingOrRunning = true
            true
        }
    }
    if (!shouldStart) {
        LogKit.i(AppStrings.ui_file_sharing_https_service_already_running_skipping_repeated_startup)
        return
    }
    ...
    try {
        ...
        try {
            LogKit.i(AppStrings.ui_start_file_sharing_raw_http_tls_service_port_arg0.format(arg0 = (port).toString()))
            ...
            LogKit.e(AppStrings.ui_file_sharing_raw_http_tls_service_failed_start_ios.format(arg0 = message), error)
        ...
        try {
            ...
            LogKit.w(AppStrings.ui_stop_file_sharing_raw_http_tls_service_failed_ios.format(arg0 = (error.message).toString()), error)
        }
        fileShareServiceLock.withLock {
            fileShareServiceStartingOrRunning = false
        }
    }
}
