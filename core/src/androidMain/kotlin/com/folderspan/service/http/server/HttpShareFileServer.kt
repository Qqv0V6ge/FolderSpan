...
actual class HttpShareFileServer private actual constructor(fileShareState: FileShareState) :
    ...
    override suspend fun downloadLocalFile(
        ...
        val target = resolveDownloadTarget(filePath)
        ...
        val presentation = resolveLinkShareFilePresentation(
            ...
            platformMimeTypeHint = target.contentType,
        )
        return respondDownload(
            ...
            inputProvider = target.inputProvider
        )
    }
    ...
    private data class DownloadTarget(
        ...
        val inputProvider: () -> InputStream?,
    )

    private fun resolveDownloadTarget(filePath: String): DownloadTarget? {
        if (filePath.startsWith("content://")) {
            ...
            runCatching {
                ...
                LogKit.w(AppStrings.ui_failed_read_content_uri_metadata_arg0_arg1.format(arg0 = filePath, arg1 = (error.message).toString()))
            }
            return DownloadTarget(fileName, fileSize, mimeType) { resolver.openInputStream(uri) }
        }

        val file = File(filePath)
        if (!file.exists()) return null
        return DownloadTarget(file.name, file.length(), "application/octet-stream") { file.inputStream() }
    }
