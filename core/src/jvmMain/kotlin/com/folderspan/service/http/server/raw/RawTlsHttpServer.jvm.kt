...
import com.folderspan.service.session.JvmDeviceSessionByteChannel
...
class RawTlsHttpServer internal constructor(
    ...
    private val schemeSwitchingProxy = com.folderspan.service.http.server.HttpPortSchemeSwitchingProxy()
    ...
    suspend fun start(port: Int) {
        var shouldCleanupOnFailure = false
        try {
    ...
    suspend fun stop() {
        val jobs = withContext(NonCancellable + Dispatchers.IO) {
            ...
            listOfNotNull(plainAcceptJob, tlsAcceptJob).also { activeJobs ->
                plainAcceptJob = null
                tlsAcceptJob = null
                ...
                try {
                    ...
                    if (client is SSLSocket) {
                        ...
                        if (!client.hasDeviceSessionAlpn()) {
                            LogKit.w(AppStrings.ui_device_session_rejected_connection_missing_alpn_arg0_remote_arg1.format(arg0 = (DEVICE_SESSION_ALPN), arg1 = (client.inetAddress?.hostAddress).toString()))
                            return@withContext
                        }
                        LogKit.i(AppStrings.ui_device_session_tls_complete_remote_arg0_alpn_arg1.format(arg0 = (client.inetAddress?.hostAddress).toString(), arg1 = (DEVICE_SESSION_ALPN)))
                        DeviceSessionServerRuntimeLauncher.run(
                            dispatcher = dispatcher,
                            channel = JvmDeviceSessionByteChannel(client),
