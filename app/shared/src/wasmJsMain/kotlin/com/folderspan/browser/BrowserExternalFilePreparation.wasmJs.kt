...
private class BrowserExternalFileSource(
    ...
    override suspend fun readRange(start: Long, endExclusive: Long): Result<ByteArray> {
        ...
        val base64 = try {
            readBrowserExternalFileRangeBase64(source, start.toDouble(), endExclusive.toDouble())
                .await<JsAny?>()
                .toString()
...
@JsFun(
    ...
    (source, start, endExclusive) => {
      ...
      const safeEnd = Math.max(safeStart, Math.floor(Number(endExclusive) || safeStart));
      if (!source || !source.slice || !source.arrayBuffer) return Promise.reject(new Error("unreadable"));
      return source.slice(safeStart, safeEnd).arrayBuffer().then((buffer) => {
...
private external fun readBrowserExternalFileRangeBase64(
    source: JsAny,
    start: Double,
    endExclusive: Double,
): Promise<JsAny?>
