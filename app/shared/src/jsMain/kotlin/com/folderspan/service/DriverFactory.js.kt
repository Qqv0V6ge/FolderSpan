...
actual class DriverFactory {
    actual fun createDriver(): SqlDriver {
        return try {
            val workerUrl =
                js("""new URL("sqljs.worker.js", import.meta.url).toString()""") as String
            LogKit.i("SQLDelight worker URL: $workerUrl")
            val worker = Worker(workerUrl)
            worker.asDynamic().onerror = { event: dynamic ->
                val message = event?.message as? String
                val filename = event?.filename as? String
                val lineno = event?.lineno as? Int
                val colno = event?.colno as? Int
                LogKit.e("SQLDelight worker error: $message ($filename:$lineno:$colno)")
            }
            worker.asDynamic().onmessageerror = { event: dynamic ->
                LogKit.e("SQLDelight worker message error: ${event?.data}")
            }
            val driver = WebWorkerDriver(worker)
            MainScope().launch {
                val schemaResult = runCatching { createSchemaWithAwait(driver) }
                schemaResult.exceptionOrNull()?.let { throwable ->
                    LogKit.e("SQLDelight schema init failed for JS worker driver.", throwable)
                }
            ...
            val message = e.asDynamic().message as? String
            LogKit.e(
                "Failed to init SQLDelight JS worker driver; falling back to noop driver. " +
...
private suspend fun createSchemaWithAwait(driver: SqlDriver) {
    ...
    when {
        userVersion == 0L && !hasTables -> {
            ...
            LogKit.i("SQLDelight schema created for JS worker driver at version $currentVersion.")
        }
        userVersion == 0L && hasTables -> {
        ...
        userVersion < currentVersion -> {
            ...
            setUserVersion(driver, currentVersion)
            LogKit.i(
                "SQLDelight schema migrated from $userVersion to $currentVersion for JS worker driver."
            )
        }
        userVersion > currentVersion -> {
            LogKit.w(
                "SQLDelight schema version $userVersion is newer than code version " +
                    "$currentVersion for JS worker driver."
            )
        }
        else -> LogKit.i(
            "SQLDelight schema already at version $currentVersion for JS worker driver."
        )
    }
}
